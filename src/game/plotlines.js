...
function suspenseArenaAntiAttractors(arenaId, relId, gravity) {
  const g = Number(gravity);
  const high = Number.isFinite(g) && g >= 80;
  const lines = [];
  if (arenaId === 'ecology') {
    lines.push(
      'THREAT_ARENA ECOLOGY: двигатель — процесс видов или среды, не одно животное. Не начинай с мха/войлока в канавах и дренаже и не с куста, который занимает поля. Ищи порчу посева, вредителя, коллапс опылителей, паразита скота, гриб древесины, цветение в водосборе, истощение почвы, стерильность семян.',
    );
    if (high) {
      lines.push(
        '80+ ECOLOGY: процесс уже бьёт основу жизни города (посев, опыление, стада, лес). Мох в канавах с нормированием — не эта полоса. Еда/вода разрешены, если это и есть система на кону, не универсальный countdown.',
      );
    }
  } else if (arenaId === 'human') {
    lines.push(
      'THREAT_ARENA HUMAN: двигатель — человеческие действия и взаимодействия: конфликт интересов, стимулы, координационный провал, обязательства, долги, статус, подражание, политическое решение, паника, бойкот, раскол, миграция, спор о собственности, несовместимые нормы.',
    );
    lines.push(
      'Тест HUMAN: убери человеческие решения — угроза продолжает развиваться сама? Тогда это НЕ HUMAN. Не используй яд, болезнь, геологию, материал, животное или погоду как настоящую причину, оставляя людей носильщиками.',
    );
    lines.push(
      'HUMAN Y — наблюдаемый факт, не «упадёт доверие»: артели рвут совместную работу перед сезоном; сотни семей не признают раздел земли; отказ повинности; один порядок наследования вытесняет другой; группа покидает район.',
    );
    lines.push('Не начинай с двух одинаково истинных памятей и не с двух реестров, которые оба верны.');
    if (high) {
      lines.push(
        '80+ HUMAN: культ, подтачивающий веру в покровителя; гражданская война групп; волна преступности; массовый отказ повинности; известное пророчество как тикающие часы (не загадка «что оно значит» — это mystery).',
      );
    }
  } else if (arenaId === 'creature') {
    lines.push(
      'THREAT_ARENA CREATURE: двигатель — конкретное существо. Популяция/гриб/экосдвиг — ECOLOGY. Не начинай с птиц, закрывающих навигационные огни.',
    );
    if (high) {
      lines.push(
        '80+ CREATURE: нашествие крылатых или существо, способное вырезать город или стада. Не «огни погасли → корабли не пришли».',
      );
    }
  } else if (arenaId === 'material') {
    lines.push(
      'THREAT_ARENA MATERIAL: двигатель в веществе. Живое в носителе — не эта арена. Не начинай с белого щелочного порошка, который несёт один ветер.',
    );
    if (high) {
      lines.push('80+ MATERIAL: вещество уже бьёт основу жизни или целостность города, не налёт на одной кровле.');
    }
  } else if (arenaId === 'built') {
    lines.push(
      'THREAT_ARENA BUILT: двигатель в устройстве человеческой системы. Если живое ломает постройку — CREATURE/ECOLOGY. Не начинай с зерна, сыплющегося в частные погреба, и не с накипи в трубах.',
    );
    if (high) {
      lines.push(
        '80+ BUILT: несущая конструкция города на грани отказа (стена, терраса, плотина, основной водосбор). Зерно/трубы — не дефолт, если жребий не про них.',
      );
    }
  } else if (arenaId === 'earth') {
    lines.push(
      'THREAT_ARENA EARTH: двигатель в ландшафте или геологии. Колодец и сток — BUILT. Не объясняй масштаб тем, что карьер меняет массу острова и остров наклоняется — это plot-shaped космология.',
    );
    if (high) {
      lines.push(
        '80+ EARTH: сила из недр, жилая терраса трещит, склон заберёт дома. Не «остров как целое теряет равновесие».',
      );
    }
  } else if (arenaId === 'sky') {
    lines.push(
      'THREAT_ARENA SKY: двигатель в атмосфере, погоде или небе. Существо в полёте — CREATURE. Не начинай с подношений и огня у обрыва как механизма погоды.',
    );
    if (high) {
      lines.push(
        '80+ SKY: остров входит в ураган или сезон уже залочен — укрепить город, укрыться, потерять незащищённые районы. Это погода на курсе, не новая физика полёта. Не ритуал у кромки.',
      );
    }
  }
  if (relId === 'contact') {
    lines.push(
      'CONTACT: ветер, споры, летающее существо, падение объекта, редкое сопряжение — можно. Не суда, не порты, не регулярный импорт. Не начинай с белого минерального порошка с одного направления ветра.',
    );
  }
  return lines;
}
...
export function formatSuspenseAnnotationAxesForPrompt(seed = {}, { recent = [], recentWindow = 5 } = {}) {
  ...
  if (history) lines.push(history, '');
  const gravity = Number(seed.gravity);
  if (Number.isFinite(gravity)) {
    lines.push(
      `GRAVITY: ${gravity} (${annotationGravityBand(gravity)}). Читай первым. Исторический вес развилки для города, не число жертв. Якоря: 0 эпизод; 50 веха; 100 «до/после».`,
    );
    lines.push(
      'Порядок: gravity → какая система города на кону → угроза, уже имеющая такой leverage → затем нынешняя ситуация. Не приклеивай раздутые исходы к мелкому давлению.',
    );
    lines.push(
      'Полосу несёт «если не предотвратить». «Если предотвратить» не обязан тянуть gravity: дефолт — отвели беду, civic-memory (праздник, молитвы, монумент, общий подъём). Не новое ведомство, протокол или инспекция.',
    );
    if (gravity >= 80) {
      lines.push(
        'GRAVITY 80+: сама угроза уже экзистенциальна — разрушение города, массовая гибель, слом веры или порядка, потеря города как целого. «Нормирование на год», потеря одного склона, гибель десятков — не эта полоса.',
      );
      lines.push(
        'Меню 80+ (еда/вода — пункты, не дефолт): нашествие крылатых; остров входит в ураган — укрепить город / укрыться; культ, подтачивающий веру в покровителя; гражданская война групп; волна преступности; сила из недр; известное пророчество как тикающие часы (не загадка смысла — это mystery). Склон, голод, токсин в пайке — тоже пункты меню.',
      );
      lines.push(
        'Self-check 80+: отрежь последние 30% текста «если не предотвратить». Сама текущая угроза всё ещё соразмерна полосе? Если нет — смени событие, не раздувай хвост.',
      );
    }
  }
  lines.push(
    'КОСМОЛОГИЯ: летающий остров 20–50 км. Нет воздушных судов, портов, регулярного импорта и межостровных маршрутов. Жизнь между сопряжениями. CONTACT можно: ветер, споры, летающее существо, падение, редкое сопряжение. Не выдумывай физику полёта острова ради сюжета. «Остров входит в ураган» — погода на курсе, не новая механика подъёма.',
  );
  lines.push(
  ...
  lines.push(
    'Причинность: мысленно A → B → C → плохой исход. Каждый переход — одно короткое предложение. Если нужно «и поэтому как-то» — перепридумай. Одна сущность не обслуживает сразу воду, скот, урожай и здания без одного простого свойства.',
  );
  lines.push(
    'Почему не закрыть сразу: самое очевидное решение и конкретная существенная цена. «Люди будут недовольны» / абстрактный религиозный конфликт — мало.',
  );
  lines.push(
    'Точка невозврата: исчез лучший вариант, цена скакнула, часть потерь неизбежна или кризис перешёл в новую фазу. Не обязана означать, что спасать город уже бессмысленно.',
  );
  lines.push(
    'Агентность: у города есть реалистичный ход — предотвратить, замедлить, локализовать, пожертвовать одним. Не чистый рок.',
  );
  lines.push(
    'Исходы обязательны: конкретное хорошее «если предотвратить» и плохое «если не предотвратить». Не generic «опасность устранена / стало хуже».',
  );
  lines.push(
    'Не используй «остров изолирован / нет внешнего подвоза» как единственный усилитель gravity: это уже канон мира, не уникальный рычаг этой угрозы.',
  );
  lines.push(
    'Вода, цистерны, дренаж, зерно, поля, голод — не универсальный субстрат countdown, если жребий про другое. Сами по себе как тема не запрещены.',
  );
  const used = new Set();
  ...
  const rel = tags.find((t) => t.groupId === 'worldRelation');
  const relId = String(rel?.tagId || '').toLowerCase();
  lines.push(...suspenseArenaAntiAttractors(arenaId, relId, gravity));
  if (relId === 'native') {
    lines.push('WORLD_RELATION NATIVE: угроза выросла из уже существующей обычной жизни острова.');
  } else if (relId === 'contact') {
    lines.push('WORLD_RELATION CONTACT: в привычную жизнь вошло новое явление, которого здесь раньше не было.');
  } else if (relId === 'legacy') {
