...
export function formatActMoveForPrompt(plot, move) {
  ...
  if (plot.storyType === 'mystery') {
    if (move.reveal === 'full') {
      ...
      lines.push(
        'Открой один-два новых факта или одну новую связь графа, не всю модель. Скрытое в хронику не выплёскивай.',
      );
    } else if (move.reveal === 'clue') {
      lines.push('Один новый факт или одна новая связь. Саму полную модель не называй.');
    } else {
      lines.push('Маска знания в этом месяце не двигается. Скрытые узлы в хронику не пиши.');
    }
  }
