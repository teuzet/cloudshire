...
export function deedConsequenceLines({ plot, applied, threat = null, averted = [], closed = false }) {
  ...
  if (alignment === 'DIRECT') {
    ...
    if (finish === 'fail' && linked) {
      ...
      lines.push(
        'Работа успела перехватить то, что уже назревало. Какую беду отвели — не называй и не пересказывай.',
      );
      lines.push(
        'Покажи это одним предметным событием: что в городе заметили, остановили или изменили, ' +
          'так чтобы было видно, зачем работа понадобилась. Не пиши «не зря» и не оценивай пользу словами.',
      );
    } else {
...
export function formatThreatPrompt({
  ...
  void dateLabel;
  const caused = (causeLines || []).some((line) => line != null && String(line).trim());
  const facts = caused
    ? [...causeLines.filter((line) => line != null && String(line).trim()), '5) Что в результате произошло', threat?.text || '—']
    : ['ПОВОД: напряжение истории дошло до края, и одна из бед случилась.', '', 'Что произошло:', threat?.text || '—'];
  const howToWrite = caused
    ? [
        'Напиши один цельный текст: что делали, что именно пошло не так и к чему это привело.',
        'Не дели запись на «сначала провал, потом событие». Событие из пункта 5 не подменяй другим.',
        'Провал покажи конкретным сбоем, а не словами «не сделали» и «не получилось».',
      ]
    : [];
  return [
    ...facts,
    '',
    ...howToWrite,
    'Заготовка события написана в будущем времени. Перепиши её как случившееся, в прошедшем времени, со своими подробностями места и людей.',
...
export function failedDeedCauseLines({ domain, process, applied }) {
  ...
  const officer = findDeedOfficer(domain, process);
  const v = actorVoice(officer);
  const who = actor ? (officer ? `${actor}, ${v.word}` : actor) : 'город сам, без названного лица';
  return [
    '1) Кто вёл',
    who,
    '2) Чего добивались',
    ...
    process?.detail || process?.summary || '—',
    '4) Исход',
    `${finishForPrompt(applied?.finish || 'fail')}. При выполнении дела что-то пошло не так, цель не достигнута.`,
  ];
}
...
export function threatTriggerLines(threat) {
  return ['Что в результате произошло:', threat?.text || '—'];
}
...
export function formatFinalePrompt({
  ...
  return [
    ...
    scale ? '' : null,
    'Если событие дано в будущем времени — перепиши его случившимся, со своими подробностями места и людей.',
