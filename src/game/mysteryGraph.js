...
export function formatMysteryMaskForPrompt(shape, { sideOpen = false, forCore = false } = {}) {
  const hard = forCore
    ? [
        ...
        'X читается без скрытых узлов: группу называй местом, должностью или именем из X, не «соседи» скрытого человека.',
    ...
    : [
        ...
        'Группу в entry и synopsis называй так, чтобы было ясно, кто это, без скрытых узлов: не голое «соседи», «его люди», «остальные».',
