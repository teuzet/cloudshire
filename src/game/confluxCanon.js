...
export function formatCityViewPrompt({
  ...
  return [
    ...
    `Зачем это было «${neighbor}», отсюда тоже не видно: что сделали и что взяли — пиши,`,
    'чужую нужду и чужой расчёт — нет, даже если в источнике они названы прямо.',
    archive
      ? `Фон архива пары (для связности; не пересказывай как новость и даты из скобок не копируй):\n${archive}`
      : '',
    'Проход в летописи уже описан: заново его не описывай и дорогу по нему не пересказывай.',
    ...
    'Подробность бери в случившемся и в своих людях, а не в описании прохода.',
