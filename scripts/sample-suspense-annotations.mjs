#!/usr/bin/env node
/**
 * Прогон suspense-брифов. Город не нужен, сохранение не пишет.
 *
 *   npm run seed-suspense-annotations -- --count 12 --out logs/plot-seeds/suspense-annotations.md
 */
...
import {
  seedSuspenseAnnotation,
  formatSuspenseAnnotationCard,
...
function renderAttempt(a) {
  ...
  if (a.annotation) {
    lines.push(formatSuspenseAnnotationCard(a.annotation));
    lines.push('');
  }
  return lines;
}
...
function renderCard(row) {
  ...
  if (row.ok && row.annotation) {
    ...
    lines.push(formatSuspenseAnnotationCard(row.annotation));
    lines.push('');
  }
...
if (args.help) {
  console.log(`Прогон suspense-брифов. Город не нужен.
...
for (let i = 1; i <= args.count; i += 1) {
  process.stdout.write(`[suspense annotation ${i}/${args.count}]\n`);
  let result = null;
  let error = null;
  try {
    result = await seedSuspenseAnnotation({ config, runtime, log });
  } catch (err) {
    error = err;
    log.warn('sample_suspense_annotation_failed', { i, error: err.message });
  }
...
const outPath = args.out
  ...
  : path.join(projectRoot(), 'logs', 'plot-seeds', `suspense-annotations-${stampName()}.md`);
...
const groups = ['truthArena', 'worldRelation', 'manifestation', 'tonePrimary'];
const labels = {
  truthArena: 'Арена угрозы (threatArena)',
...
const body = [
  ...
  `- агент: ${config.agents?.suspenseAnnotation?.model || 'suspenseAnnotation'} / судья ${config.agents?.suspenseAnnotationJudge?.model || 'suspenseAnnotationJudge'}`,
  `- лимит: один черновик + одна доработка (новый жребий не бросается)`,
  `- города нет; сохранение не писалось`,
  `- gravity ${cfg.suspenseAnnotation?.gravityMin}–${cfg.suspenseAnnotation?.gravityMax}; depth/лестница не бросаются`,
