...
| `miniCity.js` | Справочник города для мини-аппки: полосы, нависшее, вкладки |
| `annotationPool.js` / `annotationCatalog.js` | Пулы и wipe-resistant каталог |

## Клиенты

| Адрес | Кто и что видит |
|-------|-----------------|
| `/mini` | Справочник города: статы, дела, порядок, вкладки описания/брифа/хроники/людей. В Telegram — по подписи `initData`, в тестовом клиенте — по слоту `?userId=` (только когда включён `playDev`) |
| `/play` | Тестовый клиент: чат с жрецом, кнопка «город» открывает ту же мини-аппку в iframe, «данные» — отладочный инспектор со скрытыми угрозами и днями |
| `/admin` | Полная картина мира за Basic auth |

Мини-аппка в тестовом клиенте не переписана, а встроена: копия разошлась бы
с тем, что видит игрок в Telegram.

Полосы сроков («ещё недели») — язык агентов, чтобы они не считали числа.
Игроку числа наоборот нужны: справочник показывает игровые дни и рядом реальное
ожидание («ещё 15 дней (~1 ч)»), по нему игрок планирует свой онлайн.
Инспектор и админка сверх этого показывают скрытые счётчики и точные дроби.
