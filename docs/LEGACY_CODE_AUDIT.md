...
Дата: 13 сентября 2026. Снимок мёртвого, двойного и вредного кода после перехода на игровые дни и канон пары. **Применено** на ветке `conflux-days` в рекомендованном порядке: посев тайны на карточку, трёхтактные yaml-агенты и `beatPlot`, месячный бит пары, письмо месяца, пустые заглушки сопряжения, §16. Не трогались: двойная запись `etaMonths`/`tickIndex`, оболочка `runWorldTick` / `scheduler/ticks.js`, архив freeform. Документ оставлен как отчёт, не как текущее ТЗ.
